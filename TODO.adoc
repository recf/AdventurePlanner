...
** link:./AdventurePlanner.UI/ViewModels/CharacterPlanViewModel.cs#L57[57] Consider: Instead of calling AddLevel on create, instead call SetFromPlan() with initial state.
** link:./AdventurePlanner.UI/ViewModels/CharacterPlanViewModel.cs#L340[340] AddArmorPlanImpl, etc are small enough that they would probably better inline.
